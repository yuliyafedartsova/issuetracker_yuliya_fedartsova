...
<%@ include file="header.jsp" %> 
	...
	<c:set var="path" value='/issuetracker/update-issue/${issue.id}'/>
	    <c:choose>
		<c:when test="${issue.status.name eq 'Closed'}">
		<form action='/issuetracker/reopen-issue/${issue.id}'>
		<table>
		...
		<c:otherwise>
		<form action='/issuetracker/update-issue/${issue.id}'>
		<table>
		<tr><td>Summary</td>
    	<td><input type='text' name='summary'> </td> <td> ${issue.summary}</td></tr>
    	<tr><td>Description</td>
    	<td><input name="description"></td> <td>${issue.description}</td></tr>
    	<tr><td> Status </td>
    	<td><select name='statusId' size='1'>
    		<c:forEach var="status" items="${statuses}"> 
	        	<c:choose>
    	...
    	<tr><td> Resolution </td>  
    	<td><select id="res" name='resolutionId' size='1'>
    		<option></option>
    	...
    	<td><select name='versionId' size='1'>
